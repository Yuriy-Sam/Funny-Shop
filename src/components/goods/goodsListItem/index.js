  ...
  const subImageBlock = () => {
    ...
    return (
      <div className={"goodsListItem__img__block " + imgArray.classes}>
        <img
          src={
            process.env.REACT_APP_SERVER_URL + "/images/" + imgArray.imageUrl
          }
        ...
        <div className="goodsListItem__img__list">
          {images.map((item) => (
            <button
              ...
              onClick={() =>
                dispatch(
                  changeShowImg({ goodsId: getGoodsId, imageId: item.id })
                )
              }
              className={
                item.id === showImg
                  ? "goodsListItem__img__list__item _active"
                  : "goodsListItem__img__list__item"
              }
            >
              <img
                src={
                  process.env.REACT_APP_SERVER_URL + "/images/" + item.imageUrl
                }
    ...
    if (sale > 0) {
      return (
        <>
          ...
          <span>-</span>
          <p className="goodsListItem__price_new">
            {SelectCurrencyPrice(Math.floor(price - price * (sale / 100)), 1)}
          </p>
        </>
      );
    }
    ...
    <li className="goodsListItem">
      ...
      <div className="goodsListItem__body">
        <div className="goodsListItem__content">
          <Link to={`/good/${getGoodsId}`} className="goodsListItem__name">
            {name}
          </Link>
      ...
      <div className="goodsListItem__overflow">
        {changeFavoriteIcon ? (
          <button
            ...
            onClick={favoriteItemCreated}
            type="button"
            className="goodsListItem__overflow__btn addLike"
            aria-label="Close"
          >
        ...
        {changeAddIcon === 0 ? (
          <button
            ...
            onClick={addToCart}
            type="button"
            className="goodsListItem__overflow__btn addCart"
            aria-label="Close"
          >
