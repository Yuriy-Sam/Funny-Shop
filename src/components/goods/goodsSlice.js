...
const initialState = {
  ...
  urlJson: process.env.REACT_APP_SERVER_URL,
...
export const addFavoriteItemToData = createAsyncThunk(
  ...
  async (state) => {
    ...
    return await request(
      `${initialState.urlJson}/favoriteItems`,
      "POST",
...
export const removeFavoriteItemToData = createAsyncThunk(
  ...
  async (state) => {
    const { request } = useHttp();
    return await request(
      `${initialState.urlJson}/favoriteItems/${state}`,
      "DELETE"
    );
  }
);
...
export const addCartItemToData = createAsyncThunk(
  ...
  async (state) => {
    const { request } = useHttp();
    return await request(
      `${initialState.urlJson}/cartItems`,
      "POST",
      JSON.stringify(state),
      { "Content-Type": "application/json" }
    );
...
export const updateCartItemInData = createAsyncThunk(
  ...
  async (state) => {
    ...
    return await request(
      `${initialState.urlJson}/cartItems/${state.payload.id}`,
      "PUT",
...
export const removeCartItemInData = createAsyncThunk(
  ...
  async (state) => {
    ...
    return await request(
      `${initialState.urlJson}/cartItems/${state}`,
      "DELETE"
    );
  }
);
        ...
        switch (state.sortName) {
          ...
          case "lowPrice":
            state.filteredGoods.sort((a, b) => {
              let AsalePrice =
                a.sale > 0
                  ? Math.floor(a.price - a.price * (a.sale / 100))
                  : a.price;
              let BsalePrice =
                b.sale > 0
                  ? Math.floor(b.price - b.price * (b.sale / 100))
                  : b.price;
    ...
    onfilteredGoods: (state, action) => {
      ...
      if (state.goods.length > 0) {
        state.filteredGoods = state.goods.filter((item) => {
          let salePrice =
            item.sale > 0
              ? Math.floor(item.price - item.price * (item.sale / 100))
              : item.price;
          ...
          if (categoriesItems.length > 0) {
            categoriesHave = item.category.some((categories) =>
              categoriesItems.includes(categories)
            );
          } else {
            categoriesHave = true;
          }
          ...
          if (categoriesHave) {
            if (
              saleCurentyPrice > state.filterMinPrice &&
              saleCurentyPrice <= state.filterMaxPrice
            ) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchGoods.fulfilled, (state, action) => {
        ...
        state.goods = action.payload.map((item) => ({
          ...item,
          showImg: item.images[0].id,
        }));
        state.filteredGoods = state.goods;
        state.filteredGoodsCounter = state.filteredGoods.length;
        state.filteredGoods.sort((a, b) => {
        ...
        state.filterMaxInputPrice = Math.max(
          ...state.goods.map((item) => item.price)
        );
      ...
      .addCase(addCartItemToData.pending, (state) => {
      ...
      .addCase(updateCartItemInData.pending, (state) => {
        state.cartLoadingStatus = "loading";
      })
      ...
      .addCase(addFavoriteItemToData.pending, (state) => {
      ...
      .addCase(removeFavoriteItemToData.pending, (state) => {
        state.favoriteLoadingStatus = "loading";
      })
      ...
      .addCase(removeCartItemInData.pending, (state) => {
        state.cartLoadingStatus = "loading";
      })
      ...
      .addCase(removeCartItemInData.fulfilled, (state, action) => {
        ...
        state.cartItems = state.cartItems.filter((item) => {
          if (item.id == action.meta.arg) {
            ...
            console.log(
              state.totalPrice,
              "state.totalPrice removeCartItemInData"
            );
